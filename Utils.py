...
def condition(board: Board, move: Move, piece: PieceType, is_white: bool):
    ...
    board.remove_cell_piece(move.target, target_type, not is_white)
    board.remove_cell_piece(move.cell, piece, is_white)
    board.set_cell_piece(move.target, piece, is_white)
    result = is_under_check(board)
    board.remove_cell_piece(move.target, piece, is_white)
    board.set_cell_piece(move.target, target_type, not is_white)
    board.set_cell_piece(move.cell, piece, is_white)
    return result
...
def is_mate(board: Board, is_white: bool):
    ...
    if board.position_in_check:
        if (~board.get_attacks(is_white)) & board.get_king_cell_moves(king_cell) == 0:
